...
static s64 qavs3_mdate()
{
#if (defined(_WIN32) || defined(_WIN64))
    struct timeb tb;
    ftime(&tb);
    return ((s64)tb.time * 1000 + (s64)tb.millitm) * 1000;
#else
    struct timeval tv_date;
    gettimeofday(&tv_date, NULL);
    return (s64)tv_date.tv_sec * 1000000 + (s64)tv_date.tv_usec;
#endif
}
...
int main(int argc, char **argv)
{
    ...
    printf("file0: %s\n", param.org.filename);
    printf("file0: %s\n", param.rec.filename);
    fd0 = open(param.org.filename, O_RDONLY, 0);
    fd1 = open(param.rec.filename, O_RDONLY, 0);
    uint8_t *org = (uint8_t*)mmap(NULL, param.org.filesize, PROT_READ, MAP_PRIVATE, fd0, 0);
    uint8_t *rec = (uint8_t*)mmap(NULL, param.rec.filesize, PROT_READ, MAP_PRIVATE, fd1, 0);
    uint8_t *org_bak = org;
    uint8_t *rec_bak = rec;
    printf("size0: %lld\n", param.org.filesize);
    printf("size1: %lld\n", param.rec.filesize);
...
*/
    ...
    close(fd0);
    close(fd1);
    munmap(org_bak, param.org.filesize);
    munmap(rec_bak, param.rec.filesize);
#endif
	free_buffer(buf.org);
	free_buffer(buf.rec);
